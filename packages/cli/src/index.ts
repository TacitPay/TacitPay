...
const LOCAL_DEVNET_NOTICE =
  'requires local devnet (yarn env:up) — arriving with the integration-test task';
...
const runWallet = async (args: string[]): Promise<number> => {
  const [action, ...rest] = args;
  if (action === 'fund-local') {
    console.error(LOCAL_DEVNET_NOTICE);
    return 2;
  }
...
const dispatch = async (args: string[]): Promise<number> => {
  ...
  switch (command) {
    ...
    case 'demo':
      if (rest.length === 1 && rest[0] === 'seed') {
        console.error(LOCAL_DEVNET_NOTICE);
        return 2;
      }
      throw new Error(`Unknown demo command "${rest[0] ?? ''}"`);
    default:
